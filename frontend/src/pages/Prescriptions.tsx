...
import { Pill, Search, Filter, CheckCircle, X, Eye } from 'lucide-react';
...
export default function Prescriptions() {
  ...
  const [selectedPrescription, setSelectedPrescription] = useState<typeof dummyPrescriptions[0] | null>(null);

  const isPharmacist = user?.role === 'pharmacist';

  const filteredPrescriptions = dummyPrescriptions.filter(p => {
          ...
          <Card key={presc.id} hover>
            <CardBody>
              <div className="flex items-start justify-between mb-4">
                ...
                <Badge
                  variant={
                    presc.status === 'pending' ? 'warning' :
                    presc.status === 'dispensed' ? 'success' : 'danger'
                  }
                >
                  {presc.status === 'pending' ? 'Menunggu' :
                   presc.status === 'dispensed' ? 'Diterbitkan' : 'Dibatalkan'}
                </Badge>
              </div>

              <div className="space-y-2 mb-4">
                {presc.medications.slice(0, 2).map((med) => (
                ...
                {presc.medications.length > 2 && (
                  <p className="text-sm text-gray-500">
                    +{presc.medications.length - 2} obat lainnya
                  </p>
                )}
              </div>
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4 p-4 bg-gray-50 rounded-lg">
              ...
              <div>
                ...
                <Badge
                  variant={
                    selectedPrescription.status === 'pending' ? 'warning' :
                    selectedPrescription.status === 'dispensed' ? 'success' : 'danger'
                  }
                >
                  {selectedPrescription.status === 'pending' ? 'Menunggu' :
                   selectedPrescription.status === 'dispensed' ? 'Diterbitkan' : 'Dibatalkan'}
                </Badge>
              </div>
            </div>
