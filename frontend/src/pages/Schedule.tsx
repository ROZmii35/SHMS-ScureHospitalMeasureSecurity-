...
import { dummyDoctors, dummyPatients } from '../data/dummy';
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                <h2 className="font-semibold text-gray-900">
                  {selectedDoctor
                    ? `Jadwal - ${dummyDoctors.find(d => d.id === selectedDoctor)?.name}`
                    : 'Pilih dokter untuk melihat jadwal'}
                </h2>
                {selectedDoctor && (
                  <Badge variant="info">
                    {dummyDoctors.find(d => d.id === selectedDoctor)?.department}
                  </Badge>
                )}
              </div>
            </CardHeader>
